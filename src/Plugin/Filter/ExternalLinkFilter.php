...
namespace Drupal\ocha_reliefweb\Plugin\Filter;
 ...
 *
 * @Filter(
 *   id = "ocha_reliefweb_external_link_filter",
...
class ExternalLinkFilter extends FilterBase implements ContainerFactoryPluginInterface {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    $form['internal_domains'] = [
      ...
      '#default_value' => $this->settings['internal_domains'] ?? NULL,
  ...
  protected function isInternalUrl($url) {
    ...
    if (!isset($this->internalHostPattern)) {
      $internal_hosts = array_filter(preg_split('/[\s,;]+/', $domains) ?: []);
      $internal_hosts[] = $this->requestStack->getCurrentRequest()->getHost();
      $internal_hosts = array_map('preg_quote', array_unique($internal_hosts));
